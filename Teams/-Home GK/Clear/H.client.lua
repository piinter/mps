...
local v4 = CFrame.new(-1.8, 0.4, -0.3) * CFrame.fromEulerAnglesXYZ(math.pi / 0.41, 0.2, -1);
...
Mouse.KeyDown:connect(function(p3)
	...
	v9.Velocity = Player.Character.Torso.CFrame.lookVector * 25;
	v9.Velocity = v9.Velocity * Vector3.new(1, 0, 1);
	v1.AllowClick(true)
	if Player.Backpack.Module_Management.Airbourne_Tweak.Airbourne.Value == true then
		Jumped = true;
		v9.MaxForce = Vector3.new(math.huge, 1600, math.huge);
		...
		v9.MaxForce = Vector3.new(math.huge, 2100, math.huge);
		v2.TweenWeld("Right Arm", CFrame.new(1.5, 0, 0), v3, l__Enum_EasingDirection_Out__1, l__Enum_EasingStyle_Back__2, 0.4);
	...
	if Jumped == true then
		...
		v2.TweenWeld("Right Arm", v3, CFrame.new(1.5, 0, 0), Enum.EasingDirection.InOut, Enum.EasingStyle.Sine, 0.2);
