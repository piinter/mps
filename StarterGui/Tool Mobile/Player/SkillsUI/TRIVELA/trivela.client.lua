...
Tool.MouseButton1Down:connect(function()
	...
	Combo = true
	if TM.check() == "R" then
		TM.EditWeld("Right Leg", CFrame.new(0.596, -1.632, 0.997) * CFrame.fromEulerAnglesXYZ(-1.090, -0.266, -0.012))
		TM.EditWeld("Right Arm", CFrame.new(-1.351, 0.199, -0.335) * CFrame.fromEulerAnglesXYZ(1.121, 0.326, -0.324))
		TM.EditWeld("Left Arm", CFrame.new(1.228, 0.066, 0.274) * CFrame.fromEulerAnglesXYZ(-0.980, 0.274, -0.048))
	else
	...
	if curve ~= nil then
		if TM.check() == "R" then
			...
			elseif torsoAngle > math.rad(30) and (ball.Position - Player.Character["Left Arm"].Position).magnitude > (ball.Position - Player.Character["Right Arm"].Position).magnitude then
				...
				while wait() do
					if ball.Velocity.magnitude > 0.1 then
						Curve.Force = (CFrame.new(Vector3.new(0, 0, 0), ball.Velocity) * CFrame.Angles(0, math.rad(-90), 0)).lookVector * power * 5
						if power > 2 then
							power = power - 2
						else
							power = 0
