...
local function u8(p3)
	p3.Touched:Connect(function(hit)
		if hit.Name == "TPS" and hit.Anchored == false and Kick then
			...
			wait(.1)
	local force = 1
	local angle = Vector3.new(4e+006,16e+006,4e+006)
	TM.SombreroForce(hit, Vector3.new(p3.CFrame.lookVector.X * -25, p3.CFrame.lookVector.Y * 55, p3.CFrame.lookVector.Z * -25), force, angle, true)
		end;
	end);
...
script.Parent.MouseButton1Click:Connect(function()
	...
	if Result == true and Ball ~= nil then
		...
		local force = 1
		local angle = Vector3.new(4e+006,16e+006,4e+006)
		TM.SombreroForce(Ball, Vector3.new(p3.CFrame.lookVector.X * -25, p3.CFrame.lookVector.Y * 55, p3.CFrame.lookVector.Z * -25), force, angle, true)
		Plag.Disable()
	end
