...
Mouse.KeyDown:connect(function(key)
	...
	wait(0.15)
		TM.EditWeld("Right Leg", CFrame.new(0.7,-1.8,0.5) *  CFrame.fromEulerAnglesXYZ(math.pi/-3,-1.5,-0.7))
	else
		TM.EditWeld("Left Leg", CFrame.new(-0.5,-1.5,-1) *  CFrame.fromEulerAnglesXYZ(math.pi/6,-0.3,0))
	wait(0.15)
	TM.EditWeld("Left Leg", CFrame.new(-0.7,-1.8,0.5) *  CFrame.fromEulerAnglesXYZ(math.pi/-3,1.5,0.7))
	end
	Kick = true
	wait(0.25)
	TM.ResetWelds()
...
Player.Character["Right Leg"].Touched:connect(function(hit)
	...
	local force = Player.Character["Right Leg"].CFrame.lookVector * -60
...
Player.Character["Left Leg"].Touched:connect(function(hit)
	...
	if hit.Name ~= "TPS" then return end
	if Kick == false then return end
	local force = Player.Character["Left Leg"].CFrame.lookVector * -60
	local angle = Vector3.new(6e+006,1e+003,6e+006)
	Kick = false
	game.ReplicatedStorage:WaitForChild("PassFired"):FireServer()
