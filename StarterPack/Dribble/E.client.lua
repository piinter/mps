...
SClick = script.Parent.Parent.Module_Management.ShootClick
Combo = script.Parent.Parent.Module_Management.Combo
...
Mouse.KeyDown:connect(function(key)
	...
	if Combo.Value == true then return end
	...
	if Result == true and Ball ~= nil then
		local force = Player.Character["Right Leg"].CFrame.lookVector * 27
		local angle = Vector3.new(400000,0,400000)
		Kick = false
	...
	local Ball = Plagging[2]
	if Result == true and Ball ~= nil then
		local force = Player.Character["Left Leg"].CFrame.lookVector * 27
		local angle = Vector3.new(400000,0,400000)
		Kick = false
