...
local function StartKick()
	...
	AntiR.Value = true
	if TM.check() == "R" then
		TM.EditWeld("Head", CFrame.new(0, 1.5, -0.204) * CFrame.fromEulerAnglesXYZ(-0.393, 0, 0))
		TM.EditWeld("Torso", CFrame.new(0, 0, -0.001) * CFrame.fromEulerAnglesXYZ(0.002, 0, 0))
		TM.EditWeld("Right Leg", CFrame.new(0.5, -1.632, 1.064) * CFrame.fromEulerAnglesXYZ(-1.077, -0.326, -0.096))
		TM.EditWeld("Left Arm", CFrame.new(-1.551, 0.333, -0.335) * CFrame.fromEulerAnglesXYZ(1.121, 0.326, -0.324))
		TM.EditWeld("Right Arm", CFrame.new(1.628, 0.099, 0.274) * CFrame.fromEulerAnglesXYZ(-0.980, 0.274, 0.348))
	else
		TM.EditWeld("Head", CFrame.new(0, 1.5, -0.206) * CFrame.fromEulerAnglesXYZ(-0.396, 0, 0))
		TM.EditWeld("Torso", CFrame.new(0, 0, 0) * CFrame.fromEulerAnglesXYZ(0.003, 0.785, 0))
		TM.EditWeld("Left Leg", CFrame.new(-0.5, -1.623, 1.064) * CFrame.fromEulerAnglesXYZ(-1.077, 0.326, 0.096))
		TM.EditWeld("Left Arm", CFrame.new(-1.628, 0.099, 0.451) * CFrame.fromEulerAnglesXYZ(-0.978, -0.054, -0.348))
		TM.EditWeld("Right Arm", CFrame.new(1.5, 0.333, -0.196) * CFrame.fromEulerAnglesXYZ(1.039, 0, 0.547))
	end
	power = 60
	...
	if TM.getWindUp() and TM.GetUsing() and Equipped then
		...
		if TM.check() == "R" then
			if QVal.Value == false and EVal.Value == false then
				TM.EditWeld("Head", CFrame.new(0, 1.5, -0.145) * CFrame.fromEulerAnglesXYZ(-0.098, 0, 0))
				TM.EditWeld("Torso", CFrame.new(0, 0, -0.001) * CFrame.fromEulerAnglesXYZ(0.002, 0, 0))
				TM.EditWeld("Left Leg", CFrame.new(-0.585, -1.985, -0.028) * CFrame.fromEulerAnglesXYZ(0.018, 0.032, -0.050))
				TM.EditWeld("Right Leg", CFrame.new(0.422, -1.107, -1.121) * CFrame.fromEulerAnglesXYZ(1.773, -0.339, 0))
				TM.EditWeld("Left Arm", CFrame.new(-1.408, 0.283, 0.371) * CFrame.fromEulerAnglesXYZ(-1.157, -0.102, 0.254))
				TM.EditWeld("Right Arm", CFrame.new(1.559, -0.147, -0.634) * CFrame.fromEulerAnglesXYZ(0.591, 0.894, 0.259))
			...
			if QVal.Value == false and EVal.Value == false then
				TM.EditWeld("Head", CFrame.new(0, 1.5, -0.147) * CFrame.fromEulerAnglesXYZ(-0.100, 0, 0))
				TM.EditWeld("Torso", CFrame.new(0, 0, 0) * CFrame.fromEulerAnglesXYZ(0.117, 1.571, 0))
				TM.EditWeld("Left Leg", CFrame.new(-0.422, -1.107, -1.121) * CFrame.fromEulerAnglesXYZ(1.773, 0.339, 0))
				TM.EditWeld("Right Leg", CFrame.new(0.5, -2, 0) * CFrame.fromEulerAnglesXYZ(0, 0, 0))
				TM.EditWeld("Left Arm", CFrame.new(-1.492, 0.012, -0.818) * CFrame.fromEulerAnglesXYZ(0.578, 0.894, -0.259))
				TM.EditWeld("Right Arm", CFrame.new(1.351, 0.226, 0.706) * CFrame.fromEulerAnglesXYZ(-1.396, -0.376, -0.406))
			elseif QVal.Value == true then
...
Mouse.KeyDown:connect(function(key)
	if key == "g" then
		if IsMobile and OldMobile.Value == false then return end
		StartKick()
	end
end)
...
-- Touch functionality disabled - mobile players use separate script
-- UserInputService.TouchStarted:Connect(function(touch, gameProcessed)
-- 	if not IsMobile or OldMobile.Value == true then return end
...
Player.Character["Right Leg"].Touched:connect(function(hit)
	...
	cameraDirection = (math.asin(cameraDirection.Y) * 45) + 55
	...
	if curve ~= nil then
		if TM.check() == "R" then
			...
			if torsoAngle > math.rad(18) and (ball.Position - Player.Character["Left Arm"].Position).magnitude < (ball.Position - Player.Character["Right Arm"].Position).magnitude then
				Backspin.AngularVelocity = Vector3.new(0, -75, 0) + -Player.Character.Torso.CFrame.rightVector
				Backspin.Parent = ball
				game.Debris:AddItem(Backspin, .3)
				local Curve = Instance.new("BodyForce")
				Curve.Name = "Curve"
				Curve.Parent = ball
				while wait() do
					if ball.Velocity.magnitude > 0.1 then
						Curve.Force = (CFrame.new(Vector3.new(0, 0, 0), ball.Velocity) * CFrame.Angles(0, math.rad(-90), 0)).lookVector * power * -1 * 5
						if power > 2 then
							power = power - 1.5
						else
							power = 0
			...
			elseif torsoAngle > math.rad(30) and (ball.Position - Player.Character["Left Arm"].Position).magnitude > (ball.Position - Player.Character["Right Arm"].Position).magnitude then
				...
				while wait() do
					if ball.Velocity.magnitude > 0.1 then
						Curve.Force = (CFrame.new(Vector3.new(0, 0, 0), ball.Velocity) * CFrame.Angles(0, math.rad(-90), 0)).lookVector * power * 4
						if power > 2 then
							power = power - 1.5
						else
							power = 0
