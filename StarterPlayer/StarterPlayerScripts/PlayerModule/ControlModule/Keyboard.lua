...
function Keyboard:BindContextActions()
	...
	local handleMoveForward = function(actionName, inputState, inputObject)
		if self.backwardValue ~= 0 and self.forwardValue == 0 then
			self.backwardValue = -1
		end
		self.forwardValue = (inputState == Enum.UserInputState.Begin) and -1 or 0
		if self.forwardValue == 0 and self.backwardValue ~= 0 then
			self.backwardValue = 1
		end
	...
	local handleMoveLeft = function(actionName, inputState, inputObject)
		if self.rightValue ~= 0 and self.leftValue == 0 then
			self.rightValue = -1
		end
		self.leftValue = (inputState == Enum.UserInputState.Begin) and -1 or 0
		if self.leftValue == 0 and self.rightValue ~= 0 then
			self.rightValue = 1
		end
		self:UpdateMovement(inputState)
		return Enum.ContextActionResult.Pass
